...
register_activation_hook( __FILE__, array( 'WBIZFOBU_Activator', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'WBIZFOBU_Deactivator', 'deactivate' ) );

/**
 * Boot the plugin's admin, renderer, and processor components.
 *
 * @return void
 */
...
function wbizfobu_cleanup_old_logs() {
	...
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Scheduled maintenance query on a custom plugin table; not cacheable.
	$wpdb->query(
		$wpdb->prepare(
